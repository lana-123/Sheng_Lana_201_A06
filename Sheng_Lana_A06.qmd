...
[https://github.com/lana-123/Sheng_Lana_201_A06](https://github.com/lana-123/Sheng_Lana_201_A06)
...
for(player in names(player_info)){
  # calculate points/game for each player 
...
```{r}
rounded_ppg <- sapply(points_per_game, round, digits = 2)
rounded_ppg
```
...
custom_rounding <- function(number){
  ...
  round_to <- nchar(number) - 1
  rounded_num <- round(number, digits = -round_to)
  return(rounded_num)
}

# apply function to matrix 
rounded_mat_rand <- apply(mat_rand, 1:2, custom_rounding)
rounded_mat_rand
```
...
# apply avg_temp function to the month columns only 
monthly_avg_temp <- apply(max_temps[, 3:ncol(max_temps)], 2, mean)
monthly_avg_temp
...
```{r}
# slice to ensure rows have desired state
...
```{r}
# get months only for Colorado
co_monthly_temps <- co_max_temps[ , -c(1,2)]

#apply median temp function to CO data set
median_annual_temps <- apply(co_monthly_temps, 1, median)
median_annual_temps

#find index of hottest median annual temp
hottest_co_idx <- which.max(median_annual_temps)
# use index to find corresponding year in co_max_temps dataset
hottest_co_year <- co_max_temps[hottest_co_idx, 2]

#same idea for minimum temperature 
coldest_co_idx <- which.min(median_annual_temps)
coldest_co_year <- co_max_temps[coldest_co_idx, 2]

print(paste0("The maximum median annual temperature of ", max(median_annual_temps), " occurred in ", hottest_co_year, " and the minimum median annual temperature of ", min(median_annual_temps), " occurred in ", coldest_co_year, "."))
...
```{r}
self_summer <- function (number) {
  sum <- 0
  if(number == 0) { # base case
    return(0)
  } else { # recursive case
    last_digit <- number %% 10 # get the last digit
    sum <- sum + last_digit # sum last digits
    trunc_num <- number %/% 10 # remove last digit 
    return(sum + self_summer(trunc_num)) # sum, and repeat for remaining digits
  }
}
